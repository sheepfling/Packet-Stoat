...
def test_unreal_runner_main_captures_measurements_when_no_report_exists(monkeypatch, tmp_path: Path) -> None:
    measurements = tmp_path / "grill_unreal_measurements.json"
    shared = tmp_path / "grill_unreal_engine_benchmark_report.json"
    measurements.write_text('{"results":[{"scenario":"entity_state_1x10hz","packets_per_sec":1000.0,"main_thread_apply_ms":0.1}]}\n', encoding="utf-8")
    args = run_unreal_grill_benchmark.argparse.Namespace(
        fastdis=tmp_path / "fastdis_unreal.json",
        grill_reports=[shared],
    ...
    monkeypatch.setattr(run_unreal_grill_benchmark, "parse_args", lambda argv=None: args)
    monkeypatch.setattr(run_unreal_grill_benchmark.load_local_env, "load", lambda: None)
    monkeypatch.setattr(run_unreal_grill_benchmark, "DEFAULT_RAW_BASELINE", tmp_path / "missing_raw.json")
    recorded: list[list[str]] = []

    def fake_run_step(cmd: list[str]) -> int:
        recorded.append(cmd)
        if cmd[1] == "tools/capture_grill_unreal_benchmark.py":
            shared.write_text("{}\n", encoding="utf-8")
        return 0
    ...
    assert rc == 0
    assert recorded[0][1] == "tools/capture_grill_unreal_benchmark.py"
    assert recorded[1][1] == "tools/run_engine_head_to_head_matrix.py"
