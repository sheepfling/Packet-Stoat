...
def test_refresh_generated_audit_accepts_non_ready_exit(monkeypatch) -> None:
    recorded: list[list[str]] = []

    def fake_run(cmd: list[str], cwd=None, check=False, stdout=None, stderr=None, text=None):
        recorded.append(cmd)
        return subprocess.CompletedProcess(cmd, 2, stdout="audit wrote report")

    monkeypatch.setattr(package_alpha2.subprocess, "run", fake_run)

    package_alpha2.refresh_generated_audit()
...
def test_refresh_generated_audit_raises_on_real_failure(monkeypatch) -> None:
    def fake_run(cmd: list[str], cwd=None, check=False, stdout=None, stderr=None, text=None):
