...
- `build --all-native` is the single Mac-native payload lane: it builds the
  ...
  `artifacts/reports/unity_native_matrix.json` plus `.md`.
- `orientation-verify` runs the example Unity orientation scene and writes
  `artifacts/reports/unity_orientation_verification.json` and `.md`.
...
- `install-smoke` installs the package from a temporary git repo into a clean
  Unity project and writes `artifacts/reports/unity_install_smoke.json` plus
...
- `install-matrix` is optional follow-on evidence that aggregates host
  install-smoke reports into `artifacts/reports/unity_install_matrix.json` and
  `.md`.
- `adopt-install-smoke` normalizes a Windows or Linux host report into the
  local `artifacts/reports/` bundle when you want optional portability evidence.
...
- `cross-engine-equivalence` writes the Unity-facing summary of the shared
  ...
  `artifacts/reports/unity_cross_engine_equivalence.json` and `.md`.
- `head-to-head-benchmark` writes the benchmark readiness summary to
  `artifacts/reports/unity_head_to_head_benchmark.json` and `.md`, and stays
...
- On Unity Personal hosts, runtime verification can be blocked by local editor
  ...
  `artifacts/reports/unity_runtime_verification.md` and
  `artifacts/reports/unity_editor_method.log`.
...
python tools/unity_workflow.py report --out-dir artifacts/reports
```
