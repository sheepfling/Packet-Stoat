...
func _run() -> void:
	...
	if not ClassDB.class_exists("FastDisWorld"):
		push_error("FastDisWorld extension is not loaded. Build packages/godot/fastdis_gdextension and install the wrapper plus libfastdis before running the UDP smoke test.")
		failures += 1
		quit(failures)
		return
