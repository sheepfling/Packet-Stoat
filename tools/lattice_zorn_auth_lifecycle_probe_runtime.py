    ...
    def start(self) -> None:
        ...
        while time.time() < deadline:
            ...
            if self.grpc_process.poll() is not None:
                ...
                break
            try:
                readiness_payload: dict[str, object] = {"client_id": "fastdis-auth-probe", "client_secret": "fastdis-auth-probe"}
                if self.oauth_scope_mode != "locked":
                    readiness_payload["scope"] = "entities tasks objects"
...
async def _run(repo_root: Path) -> dict[str, Any]:
    ...
    oauth_expiring_server = DualServer(repo_root=repo_root, auth_mode="oauth-dev", token="dev-token", oauth_dev_token_ttl_seconds=1)
    oauth_locked_server = DualServer(repo_root=repo_root, auth_mode="oauth-dev", token="dev-token", oauth_scope_mode="locked")
    ...
    try:
        ...
        checks.append(_check("rest_sandbox_header_without_bearer_rejected", "passed" if sandbox_only_status == 401 else "failed", json.dumps(sandbox_only_payload, sort_keys=True)))
        ...
        refresh_status, refresh_payload = _http_json(
            "POST",
            f"{oauth_server.rest_base_url}/api/v1/oauth/refresh",
            payload={"refresh_token": "demo"},
        )
        checks.append(_check("rest_oauth_refresh_route_missing", "passed" if refresh_status == 404 else "failed", json.dumps(refresh_payload, sort_keys=True)))
        ...
        checks.append(_check("rest_oauth_client_credentials", "passed" if oauth_status == 200 and bool(oauth_token) else "failed", json.dumps(oauth_payload, sort_keys=True)))
        ...
        checks.append(_check("rest_oauth_dev_expiring_token_issued", "passed" if expiring_issue_status == 200 and bool(expiring_token) and int(expiring_issue_payload.get("expires_in", 0)) == 1 else "failed", json.dumps(expiring_issue_payload, sort_keys=True)))
        ...
        checks.append(_check("rest_oauth_dev_token_before_expiry_accepted", "passed" if expiring_immediate_status == 200 else "failed", json.dumps(expiring_immediate_payload, sort_keys=True)))
        time.sleep(2.0)
        ...
        checks.append(_check("rest_oauth_dev_token_after_expiry_rejected", "passed" if expiring_after_status == 403 else "failed", json.dumps(expiring_after_payload, sort_keys=True)))
        ...
        checks.append(_check("rest_oauth_scope_locked_rejects_requested_scope", "passed" if locked_issue_status == 400 else "failed", json.dumps(locked_issue_payload, sort_keys=True)))
        ...
        checks.append(_check("rest_oauth_scope_locked_accepts_unspecified_scope", "passed" if locked_default_status == 200 else "failed", json.dumps(locked_default_payload, sort_keys=True)))
        ...
        async with grpc.aio.insecure_channel(grpc_strict_server.grpc_target) as channel:
            ...
            try:
                ...
                checks.append(_check("grpc_strict_sandbox_bearer_only_rejected", "passed" if exc.code() == grpc.StatusCode.UNAUTHENTICATED else "failed", str(exc.code())))
            else:
            ...
            checks.append(_check("grpc_strict_sandbox_header_plus_bearer_accepted", "passed" if grpc_strict.entity.entity_id == "grpc-strict-sandbox-entity" else "failed", grpc_strict.entity.entity_id))
        ...
        evidence["oauth_refresh_status"] = refresh_status
