#!/usr/bin/env python3
"""Append canonical C/C++ replay rows to shared benchmark reports."""
...
def _row_from_route(route: dict[str, Any]) -> dict[str, Any] | None:
    ...
    notes = [
        "truth-verified canonical replay route normalized from core_replay_matrix",
...
def main(argv: list[str] | None = None) -> int:
    ...
    for surface in ("c", "cpp"):
        ...
        md_path = args.out_dir / f"{surface}_engine_benchmark_report.md"
        report_path.write_text(json.dumps(updated, indent=2) + "\n", encoding="utf-8")
        if md_path.exists():
            md_path.write_text(md_path.read_text(encoding="utf-8"), encoding="utf-8")
