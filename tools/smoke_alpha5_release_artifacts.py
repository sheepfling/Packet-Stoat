...
def check_wheel_install(artifact_dir: Path) -> list[dict[str, object]]:
    ...
    wheel = wheels[-1]
    with tempfile.TemporaryDirectory(prefix="fastdis_release_wheel_") as tmp:
...
def render_markdown(report: dict[str, object]) -> str:
    lines = ["# Release Artifact Smoke Report", "", f"- status: `{report['overall_status']}`", ""]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--artifact-dir", type=Path, default=current_artifact_dir(ROOT))
...
def main() -> int:
    ...
    report = {
        "schema": "fastdis.release_artifact_smoke.v1",
        "overall_status": overall,
        "artifact_dir": str(args.artifact_dir),
        "checks": checks,
    }
    args.out_dir.mkdir(parents=True, exist_ok=True)
    json_path = args.out_dir / "release_artifact_smoke.json"
    md_path = args.out_dir / "release_artifact_smoke.md"
